...
while robot.step(timestep) != -1:
    ...
    # For now, we'll send a score message every 200 steps as a test
    step_count = getattr(robot, '_step_count', 0)
    step_count += 1
    robot._step_count = step_count

    if step_count % 200 == 0 and collection_cooldown == 0:
        send_score_message(ROBOT_ID, 1)
        collection_cooldown = 50  # Prevent spam

# Note: You'll want to add proper collection detection based on:
# 1. GPS/position data to know where the robot is
# 2. Camera/color detection to identify collectables
# 3. Proximity sensors to detect when close to objects